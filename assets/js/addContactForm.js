...
var $newLinkLi = $('<li id="test" style="list-style-type: none; margin-top: 10px"></li>').append($addCompaniesButton);
...
//Add a checkbox that show the content of the Out_At DateType Input on check
function showOutAt($companiesFormLi){
    var $showCheckBox = $('<div class="show-checkbox"><input type="checkbox" class="show-out_at"><label for="show-out_at">Ajouter une date de sortie (optionnel)</label></div>');
    $companiesFormLi.append($showCheckBox);
    var $checkbox = $showCheckBox.children().first();

    $checkbox.change(function () {
        if($(this).is(':checked')){
            //get the hidden DateType input and remove the hidden attribute.
            //append a label for the DateType input
            var $outAt = $(this).parent().prev().children().last().children();
            $outAt.removeAttr('hidden');
            $('<label for="">Date de sortie (optionnel)</label>').insertBefore($outAt);
        }else{
            //get the non hidden DateType input and add the hidden attribute
            //remove the DateType input label
            var $outAt = $(this).parent().prev().children().last().children();
            $outAt.attr('hidden', 'hidden');
            $outAt.prev().remove();
        }
    });
